...
import { LEOBOT_DIR, STATE_FILE } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  const statePath = path.join(projectRoot, LEOBOT_DIR, STATE_FILE);
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, LEOBOT_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `leobot-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
