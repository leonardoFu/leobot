...
│  │    • mcp__leobot__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── leobot.log               # Host stdout
│   └── leobot.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.leobot.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'leobot-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your leobot installation
...
LeoBot uses a hierarchical memory system based on CLAUDE.md files.
...
LeoBot has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__leobot__schedule_task]
        {
...
The `leobot` MCP server is created dynamically per agent call with the current group's context.
...
LeoBot runs as a single macOS launchd service.

### Startup Sequence

When LeoBot starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned LeoBot containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/leobot.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/leobot.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.leobot.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.leobot.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.leobot.plist

# Check status
launchctl list | grep leobot
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep leobot` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; LeoBot auto-starts container runtime but may fail |
...
- `logs/leobot.log` - stdout
- `logs/leobot.error.log` - stderr
