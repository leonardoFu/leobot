...
<dict>
    ...
    <string>${projectRoot}/logs/leobot.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/leobot.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.leobot');
  } catch {
...
/**
 * Kill any orphaned leobot node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned leobot processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/leobot.service';
    ...
    unitPath = path.join(unitDir, 'leobot.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=LeoBot Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/leobot.log
StandardError=append:${projectRoot}/logs/leobot.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned leobot processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable leobot`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start leobot`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active leobot`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-leobot.sh');
  const pidFile = path.join(projectRoot, 'leobot.pid');

  const lines = [
    '#!/bin/bash',
    '# start-leobot.sh — Start LeoBot without systemd',
    ...
    '    echo "Stopping existing LeoBot (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting LeoBot..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/leobot.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/leobot.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "LeoBot started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/leobot.log"`,
  ];
