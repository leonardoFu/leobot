...
Adds Parallel AI MCP integration to LeoBot for advanced web research capabilities.
...
- **Non-blocking Design** - Uses LeoBot scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. LeoBot already set up and running
...
Add Parallel AI MCP servers after the leobot server:
```typescript
const mcpServers: Record<string, any> = {
  leobot: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__leobot__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__leobot__send_message
   4. Use mcp__leobot__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo leobot-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep leobot  # macOS
...
- Check task runs: `tail -f logs/leobot.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.leobot` (macOS) or `systemctl --user restart leobot` (Linux)
