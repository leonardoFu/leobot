...
This skill adds Telegram support to LeoBot using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.leobot/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep leobot` (macOS) or `systemctl --user status leobot` (Linux)
...
- Check bot is started: `tail -f logs/leobot.log`
...
# Linux:
# systemctl --user stop leobot
# npm run dev
# systemctl --user start leobot
```
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.leobot` (macOS) or `npm run build && systemctl --user restart leobot` (Linux)
