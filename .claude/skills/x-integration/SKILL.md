---
name: x-integration
description: X (Twitter) integration for LeoBot. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** LeoBot v1.0.0. Directory structure may change in future versions.
...
1. **LeoBot is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep leobot (macOS) or systemctl --user status leobot (Linux)
```
...
| `LEOBOT_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/leobot.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into LeoBot, make the following modifications:
...
All paths below are relative to project root (`LEOBOT_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep leobot  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status leobot
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/leobot.log | tail -20

# Script errors
grep -i "error\|failed" logs/leobot.log | tail -20
```
...
docker run leobot-agent ls -la /app/src/skills/
```
