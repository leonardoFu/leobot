...
- Added: `gmail` MCP server alongside the existing `leobot` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `leobot` MCP server configuration is unchanged
...
## Must-keep
- The `leobot` MCP server with its environment variables
